...
import {
  pushNewProduct,
  pushNewProductV2,
} from './utils/add-product-to-cart.js'
...
} from './utils/update-product-quantity.js'
 ...
 * @returns  the updated cart with the new product added to it
 * or the new cart if the user doesn't have a cart.
...
export const addProductToCart = async (req, res, next) => {
  ...
  if (!product)
    return next({
      message: "Product isn't existed or the quantity is higher than the stock",
      cause: 404,
    })
...
export const addProductToCartEnhance = async (req, res, next) => {
   ...
   * @check if the returned value is null, then the product is not found in the cart and we will add it.
   */
  let newCartProducts = await updateProductQuantityV2(
    userCart,
    productId,
    quantity
  )
